...
3. **Create a topic** with **create_topics** if none exists. Choose the right format:
   ...
   - For private-company topic filters, set `run_conditions.standard_filters.company_type_filter` to `SmallPrivate`, `EarlyStageStartup`, `GrowthStageStartup`, and `EstablishedPrivate`; do not pass `private`.
